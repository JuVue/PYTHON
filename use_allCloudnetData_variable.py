...
def plot_LWCTimeseries(obs_data,lwcvar,lwcstr, plots_out_dir, dates, **args): #, lon, lat):
    ...
    dstr=datenum2date(dates[1])
    if pmonc == True:
        fileout = plots_out_dir + dstr.strftime('%Y%m%d') + '_Obs-UMGrid_' + '_'.join(outstr) +'_' + '_'.join(moutstr) + 'LWCTimeseries'+ lwcstr + '.png'
    else:
        fileout = plots_out_dir + dstr.strftime('%Y%m%d') + '_Obs-UMGrid_' + '_'.join(outstr) + 'LWCTimeseries'+ lwcstr + '.png'
    plt.savefig(fileout)
    plt.close()
...
def plot_lwcProfiles_split(obs_data,lwcvar,lwcstr, thresholding, plots_out_dir,dates,prof_time, **args): #, lon, lat):
    ...
    plt.subplots_adjust(top = 0.9, bottom = 0.1, right = 0.92, left = 0.08)
    ...
    for pt in range(0,len(prof_time)):
        ...
        if pt == 0
            plt.legend(bbox_to_anchor=(0.1, 0.95, 0.8, 0.015), loc=9, ncol=4)
