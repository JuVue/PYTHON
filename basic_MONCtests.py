...
def plot_basicTests( monc_data, monc_spin, plots_out_dir, moutstr, mlabel, m_out_dir, data  ):
    ...
    plt.plot(monc_data[0]['v_wind_mean'][int(st_id-4),:],monc_data[0]['zn'],label = 't=' + str(st_id - 4) + 'h')
    plt.plot(monc_data[0]['v_wind_mean'][int(st_id-1),:],monc_data[0]['zn'],label = 't=' + str(st_id - 1) + 'h')
    plt.ylabel('Z [m]')
    plt.ylim([0,2.5e3])
    plt.xlim([-5,8])
    plt.title('t=0h')
