...
if int(tt2)<10:
  ijdate = idate+'0'+tt1+'0'+tt2
elif int(tt1)<10:
  ijdate = idate+'0'+tt1+tt2
else:
  ijdate = idate+tt1+tt2
...
if len(ainx)==1440:
  ...
  clddays = np.append(cmday[ainx],dummy)
  del dummy;
...
print "cld days shapes--idate:", clddays.shape
...
fed_levs =  np.arange(0,201,25)
fed_cmap = matplotlib.cm.get_cmap('jet')
fed_norm = matplotlib.colors.BoundaryNorm(fed_levs,fed_cmap.N,clip=True)
rsnr_levs = np.arange(-30,60.1,5)
rsnr_cmap = matplotlib.cm.get_cmap('jet')
rsnr_norm = matplotlib.colors.BoundaryNorm(rsnr_levs,rsnr_cmap.N,clip=False)
...
#
#24-h picture of doppler, skewness in doppler, epsilon, fractional error, SNR profile.....................
plt.clf(); plt.close()
fig = plt.figure(figsize=(7,12)); fig.subplots_adjust(wspace=0.15, hspace=0.285)
fig.suptitle('RADAR: Dissipation rate & Doppler velocity :' + idate ,x=0.1, y=0.955,horizontalalignment='left',verticalalignment='top', fontsize=12)
ax1 = fig.add_subplot(511)
cf1 = ax1.contourf(atime, Rranges, RDOP.T, cmap=dop_cmap, levels=dop_levs, norm=dop_norm, extend='both')
ax1.plot(ctime, scldbase, linewidth=1.0, color='k', label='CBH-C')
ax1.plot(atime, scldtop1, linewidth=1.0, color='k', label='CTH-R')
plt.xlim(t1,t2); plt.ylim(h1,h2); plt.grid(True)
plt.yticks(np.arange(h1,h2+0.1,hh)); plt.xticks(np.arange(t1,t2+0.1,1))
plt.title('(a) Vertical velocity [m s$\mathregular{^{-1}}$]',size=12)
plt.ylabel('Range (km)',size=12)
plt.colorbar(cf1,shrink=0.95)
cf1.cmap.set_under('gray');   cf1.cmap.set_over('black')

ax2 = fig.add_subplot(512)
cf2 = ax2.contourf(atime, Rranges, RSKW.T, cmap=dop_cmap, levels=dop_levs, norm=dop_norm, extend='both')
ax2.plot(ctime, scldbase, linewidth=1.0, color='k', label='CBH-C')
ax2.plot(atime, scldtop1, linewidth=1.0, color='k', label='CTH-R')
plt.xlim(t1,t2); plt.ylim(h1,h2); plt.grid(True)
plt.yticks(np.arange(h1,h2+0.1,hh)); plt.xticks(np.arange(t1,t2+0.1,1))
plt.title('(b) W Skewness [m$\mathregular{^{3}}$ s$\mathregular{^{-3}}$]',size=12)
plt.colorbar(cf2,shrink=0.95)
plt.ylabel('Range (km)',size=12)
cf2.cmap.set_under('gray');   cf2.cmap.set_over('black')
...
ax3.plot(atime, scldtop1, linewidth=1.0, color='k', label='CTH-R')
plt.xlim(t1,t2); plt.ylim(h1,h2); plt.grid(True)
plt.yticks(np.arange(h1,h2+0.1,hh)); plt.xticks(np.arange(t1,t2+0.1,1))
plt.title('(c) log epsilon [m$\mathregular{^{2}}$ s$\mathregular{^{-3}}$]',size=12)
plt.ylabel('Range (km)',size=12)
plt.colorbar(cf3,shrink=0.95, format='%1.0e')

ax4 = fig.add_subplot(514)
cf4 = ax4.contourf(atime, Rranges, REPSerr.T, cmap=fed_cmap, levels=fed_levs, norm=fed_norm, extend='max')
ax4.plot(ctime, scldbase, linewidth=1.0, color='k', label='CBH-C')
ax4.plot(atime, scldtop1, linewidth=1.0, color='k', label='CTH-R')
plt.xlim(t1,t2); plt.ylim(h1,h2); plt.grid(True)
plt.yticks(np.arange(h1,h2+0.1,hh)); plt.xticks(np.arange(t1,t2+0.1,1))
plt.title('(d) Fractional epsilon Error [%]',size=12)
plt.ylabel('Range (km)',size=12)
plt.colorbar(cf4,shrink=0.95,extend='max')
cf4.cmap.set_over('k')

ax5 = fig.add_subplot(515)
cf5 = ax5.contourf(atime, Rranges, RSNR.T, cmap=rsnr_cmap, levels=rsnr_levs, norm=rsnr_norm, extend='both')
ax5.plot(ctime, scldbase, linewidth=1.5, color='k', label='CBH-C')
ax5.plot(atime, scldtop1, linewidth=1.5, color='r', label='CTH-R')
plt.xlim(t1,t2); plt.ylim(h1,h2); plt.grid(True)
plt.yticks(np.arange(h1,h2+0.1,hh)); plt.xticks(np.arange(t1,t2+0.1,1))
plt.title('(e) RADAR SNR [dB]',size=12)
plt.ylabel('Range (km)',size=14)
cb=plt.colorbar(cf2,shrink=0.95,extend='both')
cf5.cmap.set_under('gray');   cf5.cmap.set_over('black')

plt.xlabel('Time [UTC]',size=14); plt.xticks(np.arange(t1,t2+0.1,1))
del ax1; del ax2; del ax3; del ax4; del cf1; del cf2; del cf3; del cf4; del ax5; del cf5; #del ax6;
plt.savefig(outdir + 'FIG_RADARattributes_'+ijdate+'case.png', format='png', dpi=100)
del fig;
...
#new-anal...20180823...only 12-16UTC......
if idate=='20180823':
  cbase[0]=cbase[0]+0.4;cbase[99]=cbase[99]+0.3; cbase[100]=(cbase[99]+cbase[101])/2;
  cbase[101:106]=cbase[101:106]-0.03;
...
#q2
if idate=='20180905':
  cbase2 = np.zeros(len(atime[a1:a2+1])); cbase2[:]=np.nan;
  base=ctop[5:79]-0.8; cbase2[5:79]=smooth(base[::-1],5); del base;
  cbase2[5]=cbase2[5]+0.55; cbase2[6]=cbase2[6]+0.3;
  cbase2[77]=cbase2[77]+0.3; cbase2[78]=cbase2[78]+0.55;
  base=cbase[120:240]+1.2; ix=np.where(base>1.7)[0]; base[ix]=base[ix]-0.2; del ix; base[67:70]=[1.371,1.367,1.361];
  cbase2[120:240]=base; del base;
  cbase2[240:248]=cbase[240:248]+0.9;
  base=ctop[248:270]-1; cbase2[248:270]=base[::-1]; del base;
  cbase2[270:300]=smooth(ctop[270:300]-1,3);  cbase2[270:]=(cbase2[269]+cbase2[272])/2; cbase2[-1]=cbase2[-2]+0.05;
  cbase2[:90]=cbase2[:90]+0.2;
  cbase2[90:120]=cbase1[90:120]-0.4; cbase2[113:116]=[1.67,1.665,1.66]; cbase2[107:111]=[1.722,1.71,1.7,1.69];
  cbase2[120:210]=cbase2[120:210]+0.15
  scldbase[c1:c2+1][::2]=cbase2;
  scldbase[c1+1:c2+2][::2]=cbase2;

##Case-8...20180901...3to18.UTC...
if idate=='20180901':
  ix=np.where(ctop>1)[0]; ctop[ix]=ctop1[ix]; del ix;
  scldtop1[a1:a2+1]=ctop;
...
#20180906...12to24....UTC.....
if idate=='20180906':
  top=ctop[300:360]; ix=np.where(top>3)[0]; top[ix]=np.nan;
  top[4]=(top[3] + top[5])/2; top[7:11]=[0.64,0.66,0.68,0.70]; top[16:23]=top[:7];
  top[32]=(top[31] + top[33])/2; top[34]=(top[33] + top[35])/2; top[36]=(top[35] + top[37])/2;
  top[41]=(top[39] + top[42])/2; top[-1]=top[-2]; top[43:46]=top[40:43]; top[52:56]=top[-4:];
  ctop[300:360]=top;
  ctop[40:64]=ctop[40:64][::-1]-0.15;
  ctop[30:39]=ctop[30:39]+0.5; ctop[32]=(ctop[31] + ctop[33])/2; ctop[34]=(ctop[33] + ctop[35])/2;
  ctop[:30]=np.nan;ctop[:30]=cbase[:30]+0.15;
  ctop[30]=ctop[30]-0.1; ctop[39]=ctop[39]-0.1;
  #base
  cbase[69:73]=cbase[69:73]+[0.1,0.15,0.3,0.3]
  cbase[49:51]=np.mean(cbase[30:60]);
  cbase[54:56]=np.mean(cbase[30:60]);
  cbase[58]=(cbase[57] + cbase[59])/2;
  cbase[61:66]=[0.65,0.64,0.62,0.62,0.64]; cbase[70]=cbase[70]+0.2; cbase[71]=cbase[71]+0.3;
  cbase[94:97]=cbase[90:93]+0.1
  scldtop1[a1:a2+1]=ctop;
  scldbase[a1:a2+1]=cbase;
  scldbase[a1+1:a2+2]=cbase;  
...
#Case-12.....20180828...0to12UTC....
#q1-ctop
if idate=='20180828':
  ctop[19]=(ctop[17] + ctop[18])/2;   ctop[45]=(ctop[44] + ctop[46])/2
  ctop[64]=(ctop[63] + ctop[65])/2;   ctop[79]=(ctop[78] + ctop[80])/2
  ctop[94]=(ctop[93] + ctop[95])/2;   ctop[109]=(ctop[108] + ctop[110])/2
  ctop[124]=(ctop[123] + ctop[125])/2;   ctop[139]=(ctop[138] + ctop[140])/2
  ctop[154]=(ctop[153] + ctop[155])/2;   ctop[169]=(ctop[168] + ctop[170])/2
  ctop[199]=(ctop[198] + ctop[200])/2
  ctop[214]=(ctop[213] + ctop[215])/2;   ctop[229]=(ctop[228] + ctop[230])/2
  ctop[244]=(ctop[243] + ctop[245])/2;   ctop[259]=(ctop[258] + ctop[260])/2
  ctop[274]=(ctop[273] + ctop[275])/2;   ctop[289]=(ctop[288] + ctop[290])/2
  ctop[304]=(ctop[303] + ctop[305])/2;   ctop[319]=(ctop[318] + ctop[320])/2
  ctop[334]=(ctop[333] + ctop[335])/2;   ctop[349]=(ctop[348] + ctop[350])/2
  ctop[31:35]=[1.48700809,1.4630239,1.43904006,1.39107203]
  ctop[262:265]= ctop[262:265]-1.1
  top=ctop[300:360];
  ix=np.where(top>3.9)[0]; top[ix]=top[ix]-2; del ix;
  ctop[300:360]=top; del top;
  ctop[350]=(ctop[349] + ctop[351])/2;  ctop[355]=(ctop[354] + ctop[356])/2;
  ctop[339:343]=[1.947186,  1.98540792,  1.9759974,  1.964556789];
  ctop[329:331]=ctop[329:331]-0.1;  ctop[345:349]=ctop[345:349]-0.1; ctop[335:338]=ctop[335:338]-0.1
  ctop[352]=ctop[352]-0.1; ctop[356]=ctop[356]+0.1
  scldtop1[a1:a2+1]=ctop;
#q2-cbase
if idate=='20180828':
  cbase[263]=cbase[263-0.1]; cbase[261:263]=cbase[261:263]-0.25; 
  cbase[249:251]=cbase[249:251]/2;
  cbase[244:249]=cbase[239:244]; cbase[229:234]=cbase[224:229]
  cbase[153:156]=cbase[153:156]=cbase[156:159];
  cbase[88:92]=cbase[92:96]; cbase[cbase>1]=np.nan;
  cbase[30:37]=cbase[37:44];
  #
  #cbase2 = scldbase[c1:c2+1][::2];  cbase2[cbase2<0.85]=np.nan;
  cbase2=cbase1;
  cbase2[26]=1.125; cbase2[7:26]=ctop1[7:26]-0.4;
  cbase2[32:37]=cbase[32:37]+0.9; cbase2[51:55]=cbase[51:55]+0.9;  cbase2[38:44]=cbase[38:44]+1;
  cbase2[76]=cbase2[76]+0.25; cbase2[70:76]=ctop1[70:76]-0.6; cbase2[87:90]=[1.339,1.334,1.332]; cbase2[62:68]=cbase[62:68]+1
  cbase2[103:107]=cbase2[90:94]; cbase2[108]=cbase2[108]+0.1;cbase2[109:120]=ctop[109:120]-0.45
  cbase2[140:143]=[1.498,1.495,1.492]; cbase2[150:155]=ctop[150:155]-0.4; cbase2[157:180]=cbase[157:180]+1.38;
  cbase2[188:197]=cbase[188:197]+1.35;  cbase2[199:210]=cbase[199:210]+1.35;
  cbase2[210:240]=cbase[210:240]+1.3;
  cbase2[243:270]=ctop[243:270]-0.55;
  cbase2[270:300]=cbase[270:300]+1.3;
  cbase2[300:330]=cbase[300:330]+1.35;
  base=cbase[330:360]+1.3; cbase2[330:360]=smooth(base,5); del base;
  cbase2[330:332]=[1.38,1.37]; cbase2[-1]=cbase2[-1]+0.55; cbase2[-2]=cbase2[-2]+0.25;
  cbase2[228]=cbase2[228]-0.1; cbase2[233]=cbase2[233]-0.1;
  cbase2[55:63]=np.sort(ctop[55:63]-0.48); cbase2[29]=cbase2[29]+0.2; cbase2[37]=cbase2[37]-0.1;
  cbase2[14:28]=cbase2[14:28]-0.1; cbase2[63:69]=cbase2[63:69]-0.1; cbase2[78]=cbase2[78]-0.1;
  cbase2=smooth(cbase2,5)
  cbase2[-1]=cbase2[-1]+0.55; cbase2[-2]=cbase2[-2]+0.25;
  scldbase[c1:c2+1][::2]=cbase2;
  scldbase[c1+1:c2+2][::2]=cbase2;

  
#CAse13--20180919....18to24...UTC....
if ((idate=='20180919') and (str(t1)=='18')):
  ctop[4]=(ctop[3] + ctop[5])/2;  ctop[16]=(ctop[15] + ctop[17])/2;   ctop[19]=(ctop[18] + ctop[20])/2;
  ctop[34]=(ctop[33] + ctop[35])/2;  ctop[46]=ctop[46]+0.015;  ctop[49]=(ctop[48] + ctop[50])/2;
  ctop[64]=(ctop[63] + ctop[65])/2;  ctop[73]=(ctop[72] + ctop[74])/2;
  ctop[77:79]= ctop[77:79]-0.11; ctop[80:82]=ctop[80:82]-0.15; ctop[79]=(ctop[78] + ctop[80])/2;
  ctop[83:98]=cbase[83:98]+0.22; ctop[73:76]=ctop[73:76]+0.05;
  cbase[48:53]=cbase[48:53]-0.1;
  cbase[53:67]=ctop[53:67]-0.25
  scldbase[c1:c2+1][::2]=cbase;
  scldbase[c1+1:c2+2][::2]=cbase;
  scldtop1[a1:a2+1] = ctop;

#CASE13....20180919....10to14 UTC.....t2 period....
if ((idate=='20180919') and (str(t1)=='10')):
  ctop[19:35] =  ctop[3:19]; ctop[19:21]=[2.12,2.11];
  cbase[37]=cbase[37]+0.5;  cbase[39:41]=cbase1[39:41];   cbase[38]=(cbase[37]+cbase[39])/2;  cbase[41:43]=cbase[41:43]+[1.2,0.7];
  cbase[:28]=cbase1[:28]; cbase[26:28] = cbase[~np.isnan(cbase)].mean()+0.1;
  cbase[9:11] = cbase[~np.isnan(cbase)].mean()
  cbase=cbase-0.2;
  scldbase[c1:c2+1][::2]=cbase;
  scldbase[c1+1:c2+2][::2]=cbase;
  scldtop1[a1:a2+1]=ctop;
...
import scipy.io as sio
print "SAVING the CLD stats data.......idate:---", idate, cbase.shape, ctop.shape, atime[a1:a2+1].shape, clddays[c1:c2+1][::2].shape
cldmday = clddays[c1:c2+1][::2];
cldtime = atime[a1:a2+1]
##sio.savemat(outdir + 'CLDstats_'+idate+'.mat', dict([('cbase', cbase),  ('ctop',ctop),  ('cldz', ctop-cbase),  ('cldtime', atime[a1:a2+1]),    ('cldmday', cldmday)]))
#sio.savemat(outdir1 + 'CLDstats_'+ijdate+'.mat', dict([('cbase', cbase),('ctop',ctop),('cldtime', cldtime),('cldmday', cldmday)]))
print "SAVING the CLD stats END........................."
#sio.savemat(outdir1 + 'RADstats_'+ijdate+'.mat', dict([('Rranges', Rranges), ('REPS',REPS), ('atime',atime)]))
sio.savemat(outdir1 + 'LIDstats_'+ijdate+'.mat', dict([('Lranges', Lranges), ('LEPS',LEPS), ('atime',atime)]))
sys.exit()
...
ax1.plot(atime[a1:a2+1], cbase, linewidth=1.0, color='k', label='CBH-R')
#ax1.plot(atime[a1:a2+1], cbase2, linewidth=1.0, color='b', label='CBH-R')
...
#ax2.plot(ctime, scldbase1, linewidth=1.5, color='b', label='CBH-C')
ax2.plot(atime[a1:a2+1], cbase, linewidth=1.0, color='k', label='CBH-R')
#ax2.plot(atime[a1:a2+1], cbase2, linewidth=1.0, color='b', label='CBH-R')
...
ax3.plot(atime[a1:a2+1], cbase, linewidth=1.0, color='k', label='CBH-R')
#ax3.plot(atime[a1:a2+1], cbase2, linewidth=1.0, color='b', label='CBH-R')
...
ax4.plot(atime[a1:a2+1], cbase, linewidth=1.0, color='k', label='CBH-R')
#ax4.plot(atime[a1:a2+1], cbase2, linewidth=1.0, color='b', label='CBH-R')
...
ax6.plot(atime[a1:a2+1], cbase, linewidth=1.0, color='k', label='CBH-R')
...
###Saving the both the sfcb, cbct..........eps of lidar and radar...............
import scipy.io as sio
sio.savemat(outdir1 + 'EPSDOPTstats_sfcb_'+ijdate+'.mat', dict([('sfcb_dopL', sfcb_dopL),('sfcb_dopR', sfcb_dopR),('sfcb_sigL', sfcb_sigL),('sfcb_sigR', sfcb_sigR),('sfcb_skwL', sfcb_skwL),('sfcb_skwR', sfcb_skwR),('sfcb_epsL', sfcb_epsL),('sfcb_epsR', sfcb_epsR),('sfcb_tmpM', sfcb_tmpM),('sfcb_potM', sfcb_potM), ('sfcb_wndM', sfcb_wndM)]))
sio.savemat(outdir1 + 'EPSDOPTstats_cbct_'+ijdate+'.mat', dict([('cbct_dopL', cbct_dopL),('cbct_dopR', cbct_dopR),('cbct_sigL', cbct_sigL),('cbct_sigR', cbct_sigR),('cbct_skwL', cbct_skwL),('cbct_skwR', cbct_skwR),('cbct_epsL', cbct_epsL),('cbct_epsR', cbct_epsR),('cbct_tmpM', cbct_tmpM),('cbct_potM', cbct_potM), ('cbct_wndM', cbct_wndM)]))
sys.exit()
